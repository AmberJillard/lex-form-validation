...
(function () {
  ...
  /* =========================================================
   * LEX BLOG BUNDLE (consolidated)
   * Replaces these external scripts:
   * 1) change-location-links-for-news.js
   * 2) observer-pagination.js
   * 3) cms-pagination-lib.js  (FinSuite - paste full code below)
  ...
  function changeLocationLinks() {
    ...
    if (linkSeparateSecondary && generalListOfLinks) {
      if (generalListOfLinks.childNodes.length > 1) {
        if (generalListOfLinks.parentNode) generalListOfLinks.parentNode.remove();
      } else {
    ...
    tagsList.forEach((list) => {
      ...
      if (parentCardFooter) parentCardFooter.prepend(list);
      const tagsWrapper = parentCard.querySelector('.news-category__tags-wrapper');
      if (tagsWrapper) tagsWrapper.remove();
    });
  }

  // run changeLocationLinks either immediately (no pagination) or
  // when FinSuite pagination finishes rendering inside .pagination-container
  function watchPaginationThenRelocateTags() {
    const container = document.querySelector('.pagination-container');
    if (!container) { changeLocationLinks(); return; }
    const obs = new MutationObserver(() => {
      if (container.querySelector('.fs-pagination')) {
        changeLocationLinks();
      }
    });
  ...
  // =========================================================
  // (2) observer-pagination.js
  // Keeps pagination tidy & sets aria-current on active page.
  // =========================================================
  function startPaginationObserver() {
    ...
    const update = () => {
      const ul = root.querySelector('.fs-pagination ul');
      if (ul && ul.childNodes.length === 1) {
        const outer = document.querySelector('.pagination-outer');
        if (outer) outer.remove();
      } else if (ul) {
        const activeLink = root.querySelector('.fs-pagination ul .fs-pagination-active a');
        if (activeLink && !activeLink.hasAttribute('aria-current')) {
  ...
  // =========================================================
  // (3) FinSuite CMS Pagination Library
  // Paste the FULL contents of your original cms-pagination-lib.js here,
  // unchanged. It defines window.FsLibrary.
  // =========================================================

  /* ======== BEGIN FsLibrary (paste full code here, unchanged) ======== */
...
${r}
  ...
  // =========================================================
  // (5) pagination-change-focus.js
  // After a page click, focus the first visible card link.
  // =========================================================
  function onPagerClickFocusFirst(e) {
    if (e.target.tagName !== 'A') return;
    const li = e.target.closest('.fs-pagination-page');
    if (!li) return;

    const paginationBox = e.currentTarget;
    const list = paginationBox.previousElementSibling; // the collection list container
    if (!list) return;

    // First visible .w-dyn-item link after pagination change
    const firstVisibleAnchor =
      list.querySelector('.w-dyn-item[style=""] a') ||
      list.querySelector('.w-dyn-item:not([style*="display: none"]) a');

    if (firstVisibleAnchor) firstVisibleAnchor.focus();
  }
  function bindPaginationFocusHandlers() {
    document.querySelectorAll('.pagination-outer').forEach((box) => {
      box.removeEventListener('click', onPagerClickFocusFirst); // idempotent
      box.addEventListener('click', onPagerClickFocusFirst);
    });
  }

  // =========================================================
  // (6) change-position-search-box.js  (guarded)
  // =========================================================
  function changePositionSearchBox() {
    try {
      ...
      if (!searchForm || (!blogNavigation && !newsNavigation)) return;

      const TABLET = (window.BREAKPOINTS && window.BREAKPOINTS.tablet) || 991;
      const w = window.innerWidth;

      if (w >= TABLET && blogNavigation) {
        blogNavigation.prepend(searchForm);
      } else if (newsNavigation) {
        newsNavigation.after(searchForm);
      }
    } catch (err) {
      console.warn('changePositionSearchBox (guarded) error:', err);
    }
  }

  // =========================================================
  // (4) Init FinSuite pagination/loadmore for the Blog collection
  // (safe init: waits for DOM, elements, and FsLibrary)
  // =========================================================
  const LIST_SEL = '.blog-collection__list';
  const LOADMORE_SEL = '.w-pagination-next.load-more-button';
  const PAGER_CONTAINER_SEL = '.pagination-container'; // preferred slot

  let initialized = false;
  function initFsPaginationOnce() {
    if (initialized) return;
    initialized = true;

    const list = document.querySelector(LIST_SEL) || document.querySelector('.w-dyn-items');
    const next = document.querySelector(LOADMORE_SEL);
    const container = document.querySelector(PAGER_CONTAINER_SEL) || document.querySelector('.w-pagination-wrapper.pagination-2');

    if (!list || !next || !container || typeof window.FsLibrary !== 'function') {
      console.error('Init aborted: missing pieces', { list, next, container, FsLibrary: typeof window.FsLibrary });
      initialized = false;
      return;
    }

    // Hide Webflow's "previous" button (we use FinSuite pagination UI)
    const prev = container.querySelector('.w-pagination-previous');
    if (prev) prev.style.display = 'none';

    // Build a unique selector for the list
    const listSelector = list.classList.length
      ? ('.' + Array.from(list.classList).join('.'))
      : LIST_SEL;

    const grid = new window.FsLibrary(listSelector);
    ...
    grid.loadmore({
      ...
      paginate: {
        enable: true,
        itemsPerPage: 6,                              // same effective value you had
        insertPagination: PAGER_CONTAINER_SEL,        // FinSuite will render inside here
        bgColor: 'transparent',
      ...
      animation: { enable: true, effects: 'fade', duration: 400 }
    });
  }

  // ---------- boot ----------
  onReady(() => {
    // passive DOM-driven behaviors
    watchPaginationThenRelocateTags();
    startPaginationObserver();
    bindPaginationFocusHandlers();
    changePositionSearchBox();
  });

  // responsive move for the search box
  window.addEventListener('resize', changePositionSearchBox);

  // Wait for FsLibrary + key elements, then init FinSuite once
  onReady(() => {
    Promise.all([
      waitFor(() => typeof window.FsLibrary === 'function'),
      waitFor(() => document.querySelector(LIST_SEL) || document.querySelector('.w-dyn-items')),
      waitFor(() => document.querySelector(LOADMORE_SEL)),
      waitFor(() => document.querySelector(PAGER_CONTAINER_SEL) || document.querySelector('.w-pagination-wrapper.pagination-2')),
    ])
    .then(initFsPaginationOnce)
    .catch((e) => {
      console.error('FsLibrary not found or selectors missing. Check errors above.', e);
    });
  });
